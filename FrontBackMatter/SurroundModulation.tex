...
% jbednar: choose one for the whole thesis: modelled or modeled
...
%% jbednar: do any of them actually use treeshrew?  If so they don't
%% seem to say so, and it's very important to say.  If they all just
%% use one dataset, maybe just describe that one dataset here, and
%% don't talk about the dependence on the dataset until the next
%% chapter? At the moment it's confusingly not clear which dataset was
%% used for most of these results.

% jbednar: Don't you need to show an orientation map + RFs + tuning
% curves for LESPI, as for the other models, and then some long-range lateral
% connection patterns, so that we can see how the model organized
% (especially the long-range connections) before going on to analyze
% it?
% And don't you need to show that LESPI works just as well as
% SCAL/SEPI for gaussians, first?  Seems like that would be a first
% figure, then the same for the natural database, pointing out any
% important differences (RF shapes, lateral connection patterns, etc.)
% The reader has not yet seen any natural-image-trained map, so
% they'll want all the details, plus they haven't seen any self-org
% results from LESPI yet.  Seems very important to do this.
...
% jbednar: self-org on what patterns?
\begin{figure}
	\centering
...
% jbednar: report N=? or X% were rejected
...
%% jbednar: I don't get it -- why say that something will occur only
%% after the onset?  Seems trivial; what's the reason to mention it?
...
%% jbednar: Every single figure or model result needs to make it clear
...
\begin{figure}
	...
	\caption[Pop-out effect in simple flanker paradigm.]{Demonstration
      ...
      %% surround, to say whether it's really pop-out or just lack of suppression?
...
modulation effects are hard-coded. Instead, the effects emerge naturally due
to simple and general mechanisms: Hebbian learning, divisive gain control,
homeostasis, and local microcircuits composed of various cell
types. The emergence of iso-orientation facilitation and suppression 
...
These results add to the growing amount of literature suggesting that the
...
%% jbednar: Not sure if I revised this sentence accurately; please check.
