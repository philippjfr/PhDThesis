...
The full set of parameters for the SEPI model are list in Appendix
...
long-range inputs, although in reality they will provide both
...
beyond the receptive field of the neuron. To observe any real
contextual-modulation effects the visual input patterns need to
...
stimuli with explicitly generated long-range range correlations.
...
multiple datasets obtained by James Bednar meant to replicate the
visual experience of an animal, both in a laboratory environment,
dominated by high-contrast bars of the animal cages, and images of a
...
attributed to inhibition from two separate inhibitory
populations. Later on we will see that the timecourse of responses is
highly stimulus dependent, resulting from the richer set of
...
by \cite{Jones2002} in \ref{ORTC_Jones}. The result shows that the
neurons show strong iso-orientation suppression with a fairly well
defined shape, which does not vary considerably in width.
...
\ref{ORTC_ContrastDependence}, but we can directly observe how the
asymmetry in the efficacy of excitatory-excitatory and
excitatory-inhibitory connections, targeting the excitatory and Sst
populations respectively, emerges. This is evident both in the
...
(see Figure~\ref{LESPI_SizeTuning}). The property of very long-range
spatial integration by the Sst population has so far been described
only in the mouse \citep{Adesnik2012}, however we predict a cell class
with a similar function should also exist for the various reasons
outlined above.
...
map development. The existence long-range excitation also partially
disrupts the organization of the model as the initially diffuse and
isotropic connections introduce destructive long-range correlation to
the model \citep{Miikkulainen2005}.
