...
instead,
% jbednar: as suggested in some work cite{hennig}
...
%% jbednar: does that mean these two sheets will have *identical*
%% activities?  If so that would mean GCAL is fine as a model, since
%% two identical items can presumably be folded into a single entity
%% for modeling.
The activation for both the excitatory and inhibitory population is
given by:
%%
%% jbednar: Should be something like
%% \eta_E = \eta_I = \frac{\eta_{A} + \eta_L}{1 + \eta_P}
\begin{equation}
...
%% jbednar: changed---or_pref is not the pref or maximum response; it's
%% an estimate of the preferred orientation.  E.g., if a neuron
%% responded equally well to 45 and 90, the reported or_pref would be 67.5, but
%% it wasn't tested at 67.5 so we don't *really* know it would respond
%% well to that -- maybe it's a bilobed tuning curve (which might be
%% found at a saddle point) with peaks at 45 and 90 and a *trough* at 67.5!
%%
...
vector product: 
%%
%% jbednar: could simplify as \sum_{i,j} instead of \sum_i \sum_j ; your choice
%% Not sure if CoG should be G and COGD should be G_d...
\begin{equation}
...
%% jbednar: what do you mean by ``capturing''?  Reproducing, in a
%% model? Extracting from the literature?  Measuring from animals?
...
%% jbednar: can you give an example of what you mean, and what it has
%% to do with being rate-based?  
Accurately capturing the response of these neurons in a rate-based
model presents a number of challenges.
%% since many of the reported properties are to do with patterns of spikes?
...
%% This what? evidence?  Unclear.
provides a great opportunity to establish whether
%% jbednar: not sure what you mean
...
\begin{figure}
	...
	\caption{Parameter exploration to determine how the linearity and
      ...
      %% jbednar: added concluding sentence; all figures should have
      %% them.  Maybe you want to say more?
	\label{SEPILinearity}
\end{figure}
...
inhibition at others, a principle known as
Dale's law. From this perspective the SEPI model already provides an
advantage over the SCAL model, showing how SCAL could be implemented
in neural circuitry. However we also wanted to see whether
...
%% jbednar: Should use blue/white/red colormap for RFs (where white is avg luminance)
%% I guess that means update it like SCAL's (ideally)?
\begin{figure}
	\centering
...
other measurements performed in macaque ($\approx 2.2\times$). Even more
strikingly, the model now exhibits diversity in size tuning shifts
that makes it a much better match to experimental results (see
\ref{ContrastShift}).
%% jbednar: why?
...
At this point two separate but very similar models have been introduced, SCAL and
SEPI, which differ mostly in that
...
the anatomy, but does this change actually buy us anything from a functional
...
that exhibits both long-range excitation and suppression. Thus we
...
metrics described above, are shown above in \ref{SCALStability}. The first
...
presumably the point where the additional excitation can no longer be
...
that activity is allowed to propagate horizontally from where it arrives in V1,
leading to disruptions in retinotopy. In summary, we can see that
adding strong lateral excitation to SCAL causes disruptions to the stability,
orientation selectivity, pinwheel density, and CoG distortion, causing
the eventual breakage of the entire model.
%% Anything to do with SCAL or the methods could be moved to the
%% previous chapter, but only if it's not much work.
...
shows very little disruption at equivalent levels of lateral excitation.  
The model shows basically no CoG distortion, and very little variation in
pinwheel density, stability, and LHI. The orientation selectivity, 
conversely, actually increases with both excitation and contrast,
opposite to what was observed in SCAL. Of course, even this model
begins to break down under very high contrast and strong lateral
excitation.
%%
% jbednar: previously said ``the pinwheel metric is not as robust to
% changes in contrast'' as SCAL, but I see the opposite; don't you?
% Also removed `` but absence of a homeostatic mechanism may disrupt map
% formation at low contrasts.'' for the same reason.
\begin{figure}
	\centering
...
will disrupt the development. Indeed, if we compare the effect of upregulating the
...
% jbednar: were those chronic expts?  If so say so. Any explanation
% for the discrepancy?
...
excitation arriving via long-range connections. However, at the very highest
lateral excitation and contrast values tested here, even this model had
trouble canceling out this additional excitation.  Moreover, the
training patterns used in these tests did not contain any long-range
correlations, since they were localized Gaussian patterns, and such
correlations would be a much stronger test of robustness.
...
When analyzing the model size and contrast response, we found very good
agreement with experiment, and even found that the variability in
...
\ref{ContrastShift}). This result fits very well with the idea that the PV
...
effects of decreasing the contrast, matching results found
by \cite{Nienborg2013} in mouse visual cortex.
% jbednar: not shown?

One major question in the literature has been the
...
\cite{Cardin2007} challenged this assumption, because they found that
the fast-spiking inhibitory cells in layer 4 of cat area 17 did not exhibit
...
%% jbednar: I'm now confused -- were the FS cells broader tuned or not?
%% Seems to say both.
response in this population, and similar studies had found conflicting
results \citep{Hirsch2003, Nowak2008}. In the SEPI model, the PV cells
generally display orientation tuning, but they are overall
significantly more broadly tuned than the excitatory population, which
is largely driven by the fact that the modeled cells do not have any
spiking threshold. \cite{Cardin2007} argue that studies such as
\cite{Nelson1994}, which demonstrated no widening in orientation
tuning of excitatory cells under GABAergic inactivation, suggest that
broad inhibitory tuning does not significantly contribute to
sharpening of orientation tuning. However, this argument ignores the role of
...
inhibitory population are no longer needed to significantly sharpen the
tuning of excitatory cells, as their tuning has already been encoded
in the feedforward connections (as noted by \citealt{Miikkulainen2005}).
...
suited towards mediating the kinds of tasks that are important
...
in these neurons. Including PV neurons leads to a model that is highly robust to
long-range excitation, but still exhibits the good properties of
...
%% jbednar: don't you need to show (examples of?) the self-organized
%% ``long-range'' connections, perhaps at the end of Results,
%% demonstrating that they are in fact short range and thus not
%% suitable for driving surround modulation?  That way you can point
%% towards the next chapter, where this problem will be resolved.
%% Right now, it's very hard for people to see what's wrong with SEPI;
%% it sounds perfect from what you show!

%% jbednar: probably will need revising after addressing the above comment.
