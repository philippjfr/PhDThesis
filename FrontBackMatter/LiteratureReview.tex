...
\begin{figure}
	\centering \includegraphics[width=0.9\textwidth]{solomon.pdf}
	\caption[The early visual pathway in primates. Reprinted from
          \cite{Solomon2007}.]% {The early visual pathway in
        primates(at least superficially the same for most other
        mammals) from the retina to the primary visual cortex (V1) via
        the lateral geniculate nucleus (LGN) of the thalamus. The left
        panel shows the pathway, while the right panels highlight
        noteworthy sections including the structure of the retina, the
        LGN and V1 broken down into their different layers and showing
        different cell types. Reprinted from Solomon et al. 2007
        \cite{Solomon2007}.}
	\label{VisualSystem}
\end{figure}
...
\begin{figure}
	...
	\caption[The center-surround receptive field structure of some
          retinal ganglion cells and LGN neurons. Adapted from
          \cite{Bear2006}.]% {The centre-surround receptive field
...
\begin{figure}
	...
	\caption[Orientation preference map and magnifications
          thereof. Adapted from \cite{Bosking1997}.]% {A) Orientation
        preference map in a ferret generated by overlaying the
        activity maps for different orientations and artificially
        colouring each area according to the orientation preference
        laid out in the legend below. The image also highlights three
        recurrent features of orientation maps in white. The square
        highlights a saddle point, where a patch of cortex selective
        for a particular direction is almost bisected by a patch
        selective to another direction. The circle highlights a
        pinwheel arrangement, where different orientations preference
        patches are arranged in a circular shape. Finally the
        rectangular shape highlights a linear zone in which
        orientation preference change continuously. B) Magnifications
        of a linear zone and two pinwheel arrangements. Adapted from
        \cite{Bosking1997}.}
...
\begin{figure}
	...
	\caption[Gabor patches at different orientations.]% {Gabor
        Patches at 0 degree and 90/180 degree orientations with
        clearly visible ON (white) and OFF (black) regions.}
	\label{Gabor}
\end{figure}
...
\begin{figure}
	...
	\caption[Development of orientation map in ferret visual
          cortex. Adapted from \citep{Chapman1996}.]% {Development of
        orientation map in ferret visual cortex from postnatal day 31
        to 42 revealed using chronic optical imaging of intrinsic
...
pathways, which were identified in the cat. As in the
\cite{Spear1994} study they found little difference in RF center radii
between parvo- and magnocellular neurons. They also found that
magnocellular neurons had greater nonlinearity indices but could find
no compelling evidence that magnocellular neurons can be classified
into distinct linear (X) and nonlinear (Y) types. There was a tendency
for parvocellular neurons to exhibit greater spatial resolution and
the highest temporal resolution to be magnocellular. This seems to
support the general conclusion reached by \cite{Derrington1984}, which
had additionally concluded that magno- and parvocellular neurons can
be further identified by their chromatic properties.  Finally, their
...
\begin{tabular}{ l | c c c c }
  \hline \hline & Eccentricity ($\degree$) & $f_c$ (cycles/$\degree$)
  & $r_c$ ($\degree$) & $k_s$ \\ \hline Magnocellular neurons & & & &
  \\
  \hspace{2mm} ($n$) & (43) & (33) & (33) & (33) \\
  \hspace{2mm} Mean $\pm$ SD & 2.71 $\pm$ 1.37 & 3.55 $\pm$ 1.94 &
  0.089 $\pm$ 0.070 & 0.68 $\pm$ 0.37 \\ \hline Parvocellular neurons
  & & & & \\
  \hspace{2mm} ($n$) & (110) & (82) & (82) & (82) \\
  \hspace{2mm} Mean $\pm$ SD & 2.24 $\pm$ 1.13 & 4.57 $\pm$ 2.75 &
  0.069 $\pm$ 0.076 & 0.52 $\pm$ 0.35 \\ \hline
\end{tabular}
\caption[Optimal spatial frequency and RF center radius.]% {Optimal
spatial frequency ($f_c$), RF center radius ($r_c$) and relative
strength of surround mechanism ($k_s$) of magno- and parvocellular
neurons in central $5\degree$ of visual space. Taken from Levitt et
al. 2001 \cite{Levitt2001}.}
\label{LevittLGN}
\end{table}
...
\noindent where $R_0$ represents the spontaneous baseline activity and
$R_e$ and $R_i$ represent the spatial weighting function of the
excitatory and inhibitory Gaussians, which take the following form

\begin{equation}
R^\prime = C . K\left[1-e^{-\frac{f}{2\sigma^2}}\right]
\label{DoG2}
\end{equation}

\noindent where $C$ is the stimulus contrast, $K$ is the peak
sensitivity, $f$ is the spatial frequency and $\sigma$ the space
constant (radius of the Gaussian at $1/e$ of the peak). The results
are shown in Figure \ref{LGNRF} and summarized in Table
\ref{SceniakLGN}.

\begin{figure}
	\centering
        \includegraphics[width=1.0\textwidth]{sceniak2006_distributions.pdf}
	\caption[Spatial extents of LGN excitatory cRF and suppressive
          surround nCRF.]% {Distributions of spatial extents iDoG
        estimates for LGN excitatory CRF and ECRF suppressive surround
        and and Suppression Index (SI) as by their laminar and
        chromatic classification. Magnocellular ($n=51$),
        Parvocellular ($n=32$), Koniocellular ($n=27$), Layers 4A,
        4C$\alpha$, 4C$\beta$ ($n=70$). Taken from
        \cite{Sceniak2006}.}
	\label{LGNRF}
\end{figure}
...
\begin{tabular}{ p{3cm} | c | c | c | c }
  \hline \hline & $r_e^{crf}$ ($\degree$) & $r_i^{crf}$ ($\degree$) &
  $r_i^{ecrf}$ ($\degree$) & SI ($k_ir_i/k_er_e$) \\ \hline
  Magnocellular (\~{x}) & 0.5 & - & 0.76 & 0.92 (0.87,0.97)
  \\ Parvocellular (\~{x}) & 0.53 & - & 1.0 & 0.84 (0.74,0.94)
  \\ Koniocellular (\~{x}) & 0.41 & - & 0.6 & 0.89 (0.82,0.95)
  \\ \hline Layer 4A (\~{x}; $n$=17) & 0.22 (0.15,0.28) & - & 0.50
  (0.30,0.73) & 0.91 (0.73,1.10) \\ Layer 4C$\alpha$ (\~{x}; $n$=35) &
  0.51 (0.34,0.65) & - & 0.62 (0.30,0.90) & 0.90 (0.84,0.96) \\ Layer
  4C$\beta$ (\~{x}; $n$=13) & 0.46 (0.17,0.76) & - & 0.51 (0.15,0.85)
  & 0.99 (0.97,1.00) \\ \hline \hline Mean (\={x}; SI\textgreater0.1;
  $n$=76) & 0.53 $\pm$ 0.06 & 1.4 $\pm$ 0.14 & 1.3 $\pm$ 0.26 & 0.85
  $\pm$ 0.22 \\ \hline Mean (\={x}; $n$=136) & 0.58 $\pm$ 0.04 & - &
  1.3 $\pm$ 1.90 & 0.65 \\ \hline
\end{tabular}
\caption[\cite{Sceniak2006} estimated spatial RF properties.]% {Data
on spatial RF properties of macaque LGN neurons between $2-5\degree$
of visual space as estimated using the DoG (see \ref{DoG1}) and
\ref{DoG2} and iDoG (see \ref{iDoG}) models. The CRF exitatory radius
($r_e^{crf}$), ECRF inhibitory radius ($r_i^{ecrf}$) and SI were
estimated from the data using the iDoG model, while the $r_i^{crf}$
was estimated using the DoG model. The estimates are broken down by
chromatic and laminar classification. The values are given as either
medians or means, where \~{x} is the median with with 95\% confidence
intervals (CI) given as (LCI,UCI) and \={x} is the mean $\pm$
SD. Suppression Index (SI) is the ratio of the area under the
inhibitory component over the area under the excitatory
component. Compiled from \cite{Sceniak2006}.}
\label{SceniakLGN}
\end{table}

The results from \cite{Sceniak2006} represent the best estimates of
...
results (shown in table \ref{V1_histology}) show a close match between
mRF and hsRF sizes as estimated in V1 and sizes of the RF center and
RF center + surround as measured in LGN, once again reaffirming the
idea that the mRF and hsRF are primarily driven by geniculocortical
afferents. The latest review \citep{Angelucci2006} has measured the
size of the hsRF in V1 neurons of macaques at 2-8$\degree$
eccentricity as having mean of about 1$\degree \pm$ 0.1, which is
roughly 2.2x larger than the mRF of the same cell based on results
from \cite{Angelucci2002} and \cite{Levitt2002}. Estimates from the
latest anatomical study summarized in table \ref{V1_histology}
provides slightly higher estimates with means of 1.09$\degree$ and
1.41$\degree$ in layer 4C$\alpha$ and 4A/C$\beta$ respectively.

\begin{table}
\centering
\begin{tabular}{c | c c c | c c c | c c | c c}
  \hline \hline & \multicolumn{6}{c|}{V1 injection ARF size
    ($\degree$)} & \multicolumn{4}{|c}{LGN label ARF size ($\degree$)}
  \\ \cline{1-7} \cline{8-11} & \multicolumn{3}{c}{mRF} &
  \multicolumn{3}{|c}{hsRF} & \multicolumn{2}{|c|}{RFc} &
  \multicolumn{2}{|c}{RFc+s} \\ \cline{1-7} \cline{8-11} Ecc. &
  L4C$\alpha$ & L4A/4C$\beta$ & L6 & L4C$\alpha$ & L4A/4C$\beta$ & L6
  & L1 & L6 & L1 & L6 \\ \hline 3 & 0.67 & 0.69 & 0.66 & 1.02 & 1.32 &
  1.19 & 0.67 & 0.77 & 0.91 & 1.02 \\ 2.5 & 0.53 & 0.55 & 0.72 & 0.89
  & 1.20 & 1.36 & 0.53 & 0.54 & 0.77 & 0.78 \\ 2.75 & - & 0.56 & 0.72
  & - & 1.31 & 1.46 & - & 0.51 & - & 0.76 \\ 4.2 & 0.75 & 0.84 & 0.73
  & 1.12 & 1.44 & 1.59 & 0.68 & 0.57 & 0.93 & 0.83 \\ 5 & 0.76 & 0.78
  & - & 1.13 & 1.35 & - & 0.66 & 0.61 & 0.91 & 0.89 \\ 8 & - & 1.21 &
  - & - & 1.72 & - & - & 1.14 & - & 1.46 \\ 5.2 & 0.88 & 0.97 & 0.88 &
  1.27 & 1.55 & 1.78 & 0.77 & 0.83 & 1.03 & 1.11 \\ \hline Mean & 0.72
  & 0.8 & 0.74 & 1.09 & 1.41 & 1.48 & 0.66 & 0.71 & 0.91 & 0.98 \\ SD
  & 0.058 & 0.089 & 0.037 & 0.063 & 0.066 & 0.1 & 0.038 & 0.085 &
  0.041 & 0.094 \\ \hline
\end{tabular}
\caption[Taken from \cite{Angelucci2006a}.]% {Visuotopic Extent of
Feedforward Connections from LGN to V1 in a number of macaque subjects
as measured using retro- and antero-grade staining between
2.5-8$\degree$ eccentricity. Taken from \cite{Angelucci2006a}.}
\label{V1_histology}
\end{table}
...
\end{tabular}
\caption[Taken from \cite{Angelucci2002}.]% {Extents of macaque V1
...
\begin{tabular}{l | c c c}
  \hline \hline Connections & Mean $\pm$ SD & Range & Anisotropy ratio
  \\ \hline FB V2 $\rightarrow$ V1 ($n = 3$) & 6.8 $\pm$ 0.4 & 6.4-7.6
  & 2 $\pm$ 0.1 \\ FB V3 $\rightarrow$ V1 ($n = 3$) & 13.4 $\pm$ 0.5 &
  12.9-13.9 & 2 $\pm$ 0.2 \\ \hline
\end{tabular}
\caption[Taken from \cite{Angelucci2002}.]% {Cortical extents of
macaque FB connections to V1 between 2.5-7.5$\degree$ eccentricity
broken down by layer given as $D_{mm}$ along V1's elevation
axis. Adapted from \cite{Angelucci2002}.}
\label{FBExtents}
\end{table}
...
\begin{figure}
	...
	\caption[3D plot of Mexican Hat connectivity.]% {3D plot of
...
\begin{figure}
	...
	\caption[Axon terminal density of excitatory and basket cells
          in V1 .]% {Summary schematic comparing relationship between
...
\begin{figure}
	...
	\caption[Somatostatin integrative field.]% {Schematic
        illustration of the cortical circuit in layer 2/3 contributing
        to surround suppression. As a visual stimulus expands (larger
        stimuli are shown in lighter grey), recruitment of adjacent
        PCs increases Sst-ir excitation through horizontal axons
        (horizontal arrows). Reproduced from \cite{Adesnik2012}.}
	\label{som}
\end{figure}
...
\subsection{Vasointestinal peptide expressing interneurons}
...
\subsection{Somatostatin and Parvalbumin expressing interneurons}

The previous review provided a detailed review of the literature on
Somatostatin-expressing (Sst) interneurons, summarizing them as a
population of inhibitory cells with a delayed response and
feature-selective inhibition, which may be involved in long-range
surround suppression and disinhibition of the feedforward
pathway. Recent studies, have further refined this story, indicating
that while Sst neurons may be involved in orientation dependent
surround suppression in the superficial layers \citep{Adesnik2012},
their inactivation has little effect on surround suppression in deeper
layers \citep{Self2014}. Although, a recent study looking at the
contribution of feedback on surround suppression has shown that by
cooling extrastriate visual cortex, surround suppression could be
significantly reduced, these results also seem to confirm that there
is a strong source of tuned surround suppression within striate cortex
\citep{Nassi2013}. The precise origin of delayed orientation-tuned
suppression in primary visual cortex therefore remains unclear,
however current evidence indicates that both somatostatin neurons and
cortical feedback contribute to this effect.

Another study in auditory cortex once again confirmed the previously
described properties of Sst and Parvalbumin (PV) expressing
interneurons, hypothesizing that `while PV neurons may provide broadly
tuned feedforward inhibition for a rapid control of ascending inputs
to excitatory neurons, the delayed and more selective inhibition from
SOM neurons may provide a specific modulation of feedback inputs on
their distal dendrites` \citep{Li2014}.

Overall these results seem to confirm the general conclusions drawn
about PV and Sst-neurons in last years review, and at the very least
suggesting that no major revisions to the multi-population model
developed over the course of this PhD will be required.
...
cortex. While their work


\section{Interactions between neuromodulators and V1 circuitry}

The neuromodulatory influences on the circuitry of primary visual
cortex, described in the previous literature review are mediated by
their influence on the circuitry that is internal to the primary
visual cortex. As the prime interest of this report and the PhD
project is to characterize the influence of cholinergic modulation on
V1 circuitry, this section will detail how cholinergic basal forebrain
inputs may be modulating the different cell types described above and
how this may give rise to the effects that were described in the PhD
proposal. It will also try to provide a general framework of
...
\begin{figure}
	\centering \includegraphics[width=1.0\textwidth]{FBThiele.pdf}
...
A further controversy that has not yet been fully resolved is
...
% In terms of cholinergic modulation of the intracortical circuitry no clear framework has emerged as being superior or accounting for the whole range of experimental evidence. The two predominant theories of cortical
...
The \cite{Turrigiano2011} review provides a broad overview of the role
of synaptic and intrinsic homeostatic mechanisms in neural tissues
during experience dependent circuit refinement and their contribution
to network stability. The schematic in figure \ref{Homeostasis}, shows
how these different mechanisms can regulate the balance of excitation
and inhibition received by a neuron. In context of the ongoing
modeling work as part of this PhD, it is particularly interesting that
high network activity can increase the excitatory currents onto
GABAergic interneurons. This homeostatic mechanism is driven by
activity dependent regulation of the \textit{Narp} gene, which is
secreted presynaptically by pyramidal neurons and accumulates
preferentially at excitatory synapses onto PV interneurons
\citep{Chang2010}. Such a mechanism could be employed within the
multi-population model to achieve greater robustness during
self-organization.

\begin{figure}
	\centering
        \includegraphics[width=0.7\textwidth]{Homeostasis.pdf}
	\caption[]% {Schematic representation of two distinct
        mechanisms for the homeostatic regulation of neuronal
        firing. (a) Synaptic homeostasis changes the balance of inward
        and outward voltage-dependent conductances. (b) Intrinsic
        homeostasis regulates the intrinsic excitability by altering
        the balance of inhibitory and excitatory ion
        channels. Reproduced from \cite{Turrigiano2011}.}
	\label{Homeostasis}
\end{figure}
