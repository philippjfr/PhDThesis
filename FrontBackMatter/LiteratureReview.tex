...
%% jbednar: I'm not sure you need this next bit; it derails the
%% argument a bit when presented here.
% One way that this issue can be addressed is to build theoretical
% models that span various levels of description, from the smallest
% scales of molecular interactions all the way up to the behavior of 
% the entire organism.
...
across various levels of description. This issue is precisely what David
...
contribute to the overall understanding about the system, but to
...
discrepancies). In this chapter we will summarize the
...
starting to integrate that information into models that will allow us
to make novel predictions about the physical implementation,
algorithmic operation, and computational function as well as learning and
...
spatial/phase invariance. In reality this classification is not
particularly clear, 
...
factor. Apart from phase invariance the neurons often exhibit
...
% jbednar: This paragraph is missing some citations, but maybe no one
% will notice.
...
which the neural circuit organizes. The functional organization of V1
arises during the development of the animal and is thought to be
mediated largely by activity dependent processes as the next section
will show.
% jbednar: The above statement is very controversial, and should have
% some caveats.
...
do not fully mature. Thus orientation maps and
other topographic organizations develop initially even in absence of
external visual input,
% jbednar: Removed since it would need cites and explanations, and is
% covered below: 
% perhaps through internally generated visual
% activity such as retinal waves,
but then require external stimulation to fully mature.
...
processing areas.
% cite
Some successful models of development have focused
...
prominent proposals in theory and in models have been focused on the
...
Hebbian learning \citep{Wolf}.
% jbednar: broken ref?
The LISSOM and GCAL models,
% cite
which provide the basis of the modeling work proposed in
...
% jbednar: what do you mean by ``these processes''? Activity-dependent
% weight modification?  Retinal wages?
...
direction as them, while at pinwheels short-range
connections are made with cells with a wide range of orientation
preferences \citep{Ohki2005}.
% jbednar: Okhi didn't show connectivity; what do you mean?
However, it is known that the patchy
...
% Don't review needs partial rewrite
...
\citep{Angelucci2002}. Lateral connections could therefore provide a
...
sources will be laid out and analyzed. Anatomical data suggests that the
...
In summary, there is strong evidence that lateral connections underlie
...
facilitation and low contrast suppression. Apparently unlike in other species,
layer 4 horizontal connections are isotropic but macaques do exhibit patchy
...
identified by \cite{Hirsch2003}.

\cite{Carandini2002} on the other
...
feedforward inhibition. A possible feedforward inhibition circuit is shown
...
Studies in mouse visual cortex seem to confirm
such a dual purpose of Pv-ir neurons, although they also find higher
heterogeneity in the Pv-ir population \citep{Runyan2010}. This result may
...
most mouse Pv-ir cells displayed broad orientation tuning
...
response profile, large spatial extent and relatively broad orientation tuning may allow them to carry out fast, adaptive
gain control and broadly tuned suppression and thereby sharpen the
...
\begin{figure}
	...
	\caption[Schematic proposing Sst neurons' role in integrating
      ...
      grey), recruitment of adjacent principal cells (PCs) increases Sst-ir excitation
...
The material reviewed so far has focused primarily on the two most common types of
inhibitory interneurons, the Parvalbumin (PV) and
Somatostatin-expressing (Sst) cells.  Since that initial work, a number of studies
...
% cites?  Explain why sparse coding?
Sparse
...
% jbednar: explain that this paragraph (really should be several)
% outlines a theory, and explain whose theory it is (yours?).
...
Arguably, the computational task in vision is to map visual experience to the
...
% jbednar: planning section of what?  Refer by number.
...
% jbednar: too late for this cite; move to first use of 'sparse'
...
\cite{Geisler2001} demonstrated that the performance in contour-detection tasks could be predicted by a local grouping rule derived
...
to enhance commonly encountered patterns to aid the identification of contours
and form.
...
\citep{Schmidt1997}, but conflicting results have been reported for macaque
\citep{Angelucci2002}. By performing analyses on an early tree shrew
...
found neurons that exhibited co-circularity and anti-cocircularity
...
needed if we are to understand how the patterns of lateral
connectivity relate to natural image statistics.
...
distributions, which they can scale to large cortical
...
simply a statistical model of connectivity it cannot address questions at
...
specific function. \cite{Eliasmith2012} present one such model,
...
for linking low-level
processes to the higher-level processes that these processes
underlie. Models at the other extreme are relegated to be very high level
...
literature in V1. Specifically, point, rate-based neuron models have
...
\ref{contextualmodulation}, there are two main phenomena that
...
\cite{Li2002} proposes that through recurrent enhancement of responses, long-range lateral connections can give rise to a visual
...
% Cite Sander Keemink's work too?
...
circuit as predictive coding, modeling the
response of V1 neurons using a competitive mechanism between neurons
...
have for a long time investigated the interaction between natural-image statistics and surround-modulation effects. This is perhaps one
of the most interesting approaches, because it treats contextual
phenomena not as a by-product of some underlying computational
...
homogeneity in an image between center and surround, gating the divisive
normalization component accordingly, which directly leads to feature-specific surround modulation effects observed in experiments. In this
...
All of these models capture important parts of the phenomena involved, highlighting
...
models are static, presupposing an existing profile of
connectivity, and thus cannot directly address how different cell types
could interact to give rise to a circuit, 
mechanistically explaining how these effects emerge.
...
other words, V1 is not a static circuit, organizing solely based
...
have been developed using these ideas, explaining the development
...
self-organization of neurons into topographic maps via the
simple process of Hebbian learning. The \cite{VonderMalsburg1973} model showed for the first
time that very simple mechanisms such as \ref{MexicanHat} connectivity
can
...
to show that the development of realistic stripy ocular dominance columns could
be explained, demonstrating the role of neurons competing to
...
orientation selectivity.  They argued for a strong role for recurrent
...
\citep{Wilson2010}, auditory \cite{Khan2011} and other feature maps. The
fundamental principles underlying these computations are generally the
...
known about self-organization of the cortex.

In recent years some effort has been put into making the SOM-based models more
closely represent the functional organization of the cortex, starting
with the LISSOM model \citep{Bednar2003} and more recently
the simpler and more robust GCAL \citep{Stevens2013} model. In these models the retina,
...
mechanism including contrast-gain control, an adaptive per-neuron
threshold, and lateral connectivity \citep{Stevens2013}. Extensions of
this model explaining the development of complex cells and of contrast-dependent size tuning \citep{Antolik2011} as well as a continuous time
model matching LGN/V1 temporal reaponse functions have also been
developed \citep{Stevens2011}.

The GCAL (gain control, adaptive, lateral) model is based on several core observations about
...
orientation, color and direction. Selectivity to these features is
...
different areas in the cerebral cortex are originally cytoarchitectonically
...
capable of capturing any sensory modality. While the validity of this hypothesis remains controversial, experiments such as those by
...
features usually associated with V1. This idea is behind much of what has
...
% jbednar: explain ``sheet''?
The architecture of the GCAL model in its simplest form and as it will
initially be used in this project relies on only four sets of neurons, a
retinal sheet for the presentation of stimuli, two RGC/LGN sheets and
a V1 sheet. Neurons in these sheets are connected with different intra- and
...
color \citep{Bednar2012a}. All these models are trained by presenting a visual
...
further robustness to the LISSOM model and to allow it to respond across a
wide range of contrasts, in GCAL contrast-gain control was introduced in the
...
it can explain some of the size-tuning properties of V1 neurons, as the
...
%jbednar: Don't you need to discuss Antolik:phd in detail, explaining
...
actually capture features of the visual input. Therefore the GCAL model
sits at the three-way intersection between anatomical organization,
...
computational requirements driven by a task.  These topics will be the
