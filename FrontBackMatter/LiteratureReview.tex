...
estimates so far of the spatial properties of LGN receptive fields. The first
...
estimates. This difference may be explained by the more homogeneous distribution
of cells, as the sample population was taken exclusively from layer
4. The older protocol also failed to confine the drifting sine
grating to a disk, which may have resulted systematic underestimation
of the excitatory component due to long-range suppressive effects. While
...
spatial extent of the excitatory CRF centers was found to be contrast
...
high (SI\textgreater0.8) for 80\% of cells.
...
grating disk drive should \emph{reduce} long-range influences on the response,
...
unaccounted mechanisms. The iDoG model (see \ref{iDoG})
...
kept in mind, this study is still the best attempt at controlling unaccounted
...
% jbednar: need a citation for the claim about ocular dominance, as I
% don't think this is currently believed (according to Sigrid
% Lowell).  Most 2/3 neurons are binocular, so one wouldn't expect
% major modulation of the weights by OD, except in strabismus.
...
several other species. This could also indicate that
orientation-specific contour completion
in macaques is mediated by feedback connections. Long-range horizontal
connections have been shown to elicit only subthreshold responses
\citep{Hirsch1991} and are thus limited to a modulatory
...
% jbednar: are those numbers end-to-end or radius (from soma to tip)
% measurements? Clarify that.
...
% jbednar: what are ``different inhibitory components''?  Sounds
% unmotivated here.  Perhaps ``the variety of different types of inihibitory interactions''?
...
suppression mediated by various local inhibitory circuits, it is necessary to
...
%% jbednar: Also mention VIP and that the three classes form a
%% partition, covering nearly all of the reported inhibitory cells?
%% Otherwise it sounds very arbitrary to consider two of what one
%% might imagine to be hundreds of cell types. 
...
cortex across all laminae, accounting for 42\% in layer 2/3 and layer 5
...
GABA release \citep{Freund2007,Markram2004}, ensures that basket cells are
...
%% jbednar: Don't say hypercolums, say mm (right?).  Unless you've rigorously
%% defined hypercolumn?
...
properties are consistent across both layer 2/3
...
although their excitatory inputs are weak, those inputs are facilitatory,
...
% roughly following \citet{Sillito1979}'s original proposal.?
% jbednar: right?  Sounds just like it to me.  What's the difference?
% Explain it here.
...
vasoactive intestinal peptide (Vip)-ir population discussed below. The Sst-ir cells
may therefore be involved in mediating top-down control of sensory
processing, effectively gating context-dependent processing in a 
...
of cortical pyramidal cells. These results were then affirmed
...
forebrain \citep{Wickersham2009}, the evidence suggests a strong involvement
...
\begin{figure}
	...
	\caption{Distribution of GABAergic interneurons in % jbednar: mouse?
      S1 cortex by immunohistological marker. Reproduced from \cite{Rudy2011}.}
...
% jbednar: also cite perrinet SR 2015
...
% jbednar: See ``roughly following'' above -- how's what's observed
% different from this idea? 
% Also explain that the model is only 1D, and thus cannot process
% realistic stimuli?
...
%% Still need to address comments and discuss Antolik Thesis
